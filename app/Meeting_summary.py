...
# ══════════════════════════════════════════════════════════
# 1. AUDIO & TRANSCRIPTION (EXTREME SPEED)
# ══════════════════════════════════════════════════════════
def prepare_audio(uploaded_file=None, local_path=None) -> str:
    ...
    # Use native FFMPEG via subprocess for maximum speed, bypassing slow python wrapper overhead
    try:
        command = [
            "ffmpeg", "-y", "-i", raw_path, 
            "-ac", "1", "-ar", "16000", "-vn", 
        ...
        # Fallback to older method if ffmpeg is not globally available in PATH
...
def transcribe_audio(wav_path: str, live_status_container=None):
    # Dynamic VRAM Allocation: Load Whisper ONLY for transcription, then kill it
    has_cuda = torch.cuda.is_available()
    device_type = "cuda" if has_cuda else "cpu"
    
    # Use float16 on GPU for 2x speed and half VRAM usage compared to int8
    # float16 is natively supported by RTX 3050 and eliminates quantization overhead
    compute_type = "float16" if has_cuda else "int8"
    
    logging.info(f"Loading Whisper into {device_type.upper()} with {compute_type} precision...")
    from faster_whisper import WhisperModel
    
    # Using 'base' model for speed, as per the 10/10 pipeline
    model = WhisperModel("base", device=device_type, compute_type=compute_type, cpu_threads=8)
    ...
    segments, info = model.transcribe(
        ...
        task="translate"  # <--- ADD THIS EXACT LINE HERE
    )

    segments_list = []
    for segment in segments:
        segments_list.append(segment)
        # 🚨 KEEPALIVE MECHANISM: Updating the UI prevents the browser connection from timing out during long 1-2 hour meetings!
        if live_status_container:
            # We show the latest transcribed text to keep the WebSocket active
    ...
    duration_seconds = info.duration  # Accurate duration from info object
    
    # 🚨 CRITICAL VRAM HANDOFF 🚨
    # Delete model and flush GPU memory so Ollama can use the full RTX 3050 VRAM without OOM crash
    logging.info("Transcription complete. Flushing VRAM for Ollama...")
    del model
    if has_cuda:
        gc.collect()
...
def process_transcript(transcript: str, progress_bar, status_text, language: str) -> str:
    ...
    # Maximize 8K token context (Gemma) by explicitly allocating 8192 tokens in Ollama
    # Chunk size set to 20000 (~5000 tokens) to safely leave 3192 tokens for LLM generation
    ...
    for i, chunk in enumerate(chunks):
        ...
        map_prompt = f"""
        Read the following meeting transcript chunk. Output ONLY a concise, bulleted list of raw facts, decisions, and action items.

        STRICT RULES:
        1. EXTREME SHORTHAND: Write as little text as possible. Do not write full paragraphs.
        2. NO FLUFF: Ignore pleasantries and conversational filler.
        3. SOFT ATTRIBUTION: If a specific person makes a major demand or decision, note their name. Otherwise, focus on the fact itself.
        4. EXTRACT METRICS (IF PRESENT): Capture specific numbers, deadlines, or timelines.
        5. EXTRACT BUSINESS RISKS (IF PRESENT): Note any discussions around legal liability, document approval statuses, security severities, or compliance.
        6. ACTION ITEMS / OWNER: Try your absolute best to identify the speaker or owner of a task. If a specific person's name is mentioned, use it! Only use "Team" if absolutely no one is specified.
        7. LANGUAGE: Write all your notes strictly in English. Do not translate to English unless the meeting was in English.
    ...
    2. CONTEXTUAL NAMES: Use names in the Discussion Points only if they add important context (e.g., "Chris clarified that..."). If not, describe the agreed-upon fact.
    ...
    Action Items
    1. **[Short Action Title]** - [Name of Owner (or 'Team')]: [Specific Task Details]
    2. **[Short Action Title]** - [Name of Owner]: [Specific Task Details]
...
def generate_pdf(summary: str, duration_sec: float) -> str:
    ...
    try:
        ...
        # Insert horizontal dividers above all headers
        html_content = html_content.replace('<h2>', '<hr class="section-divider"><h2>')
        
        # Process Action Items into beautiful boxes
        def convert_action_list(list_content):
            # Make the owner name colored using regex targeting: <strong>Task</strong> - Owner:
            list_content = re.sub(r'(<strong>.*?</strong>\s*-\s*)([A-Za-z0-9\s\(\)\'\.\_]+?):', r'\1<span class="owner">\2</span>:', list_content)
            # Replace <li> with action-item-box while keeping it an li
            boxes = re.sub(r'<li>(.*?)</li>', r'<li class="action-item-box">\1</li>', list_content, flags=re.DOTALL)
            return boxes
            
        # Safely capture the list (<ul> or <ol>) immediately following Action Items without swallowing other sections
    ...
    if audio_path_to_process:
        ...
        if st.session_state.transcript is None:
            if st.button(":arrow_forward: Initialize Transcription", type="primary", use_container_width=True):
                ...
                live_text.empty() # Clear the live text once done
                status_container.success("Transcription complete. VRAM freed.")

    if st.session_state.transcript is not None:
        st.divider()
        col1, col2 = st.columns(2)
        with col1:
            # Client-side HTML download link to prevent Streamlit rerun interrupting the summary process
            import base64
            b64_transcript = base64.b64encode(st.session_state.transcript.encode('utf-8')).decode()
            dl_html = f'''
            <a href="data:text/plain;charset=utf-8;base64,{b64_transcript}" download="{filename_display}_transcript.txt" 
               style="display: block; padding: 0.5rem 1rem; color: #FFFFFF; background-color: #2b313e; 
               border-radius: 0.5rem; text-decoration: none; text-align: center; width: 100%; 
               box-sizing: border-box; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; 
               border: 1px solid rgba(250, 250, 250, 0.2); font-weight: 500;">
...
#     logging.info("Executing ultra-fast transcription sequence...")
#     # Added back hallucination reduction parameters that DO NOT impact speed heavily (condition_on_previous_text=False)
#     segments, info = model.transcribe(
#         wav_path,
#         beam_size=1,            
#         vad_filter=True, # Critical for 1-2 hour meetings
#         vad_parameters=dict(
#             min_silence_duration_ms=500
#         ),
#         condition_on_previous_text=False, # This is the biggest anti-hallucination fix for long audio
#         without_timestamps=True  # 🚀 HUGE SPEEDUP: Skips predicting timestamps
#     )
...
#         5. EXTRACT BUSINESS RISKS (IF PRESENT): Note any discussions around legal liability, document approval statuses, security severities, or compliance.
#         6. ACTION ITEMS / OWNER: Try your absolute best to identify the speaker or owner of a task. If a specific person's name is mentioned, use it! Only use "Team" if absolutely no one is specified.
#         7. LANGUAGE: Write all your notes strictly in {language}. Do not translate to English unless the meeting was in English.
...
#     1. ANTI-HALLUCINATION: You must ONLY use the information provided in the "Extracted Shorthand Notes" below. Do NOT invent fake Service Level Agreements (SLAs), threat levels, or names.
#     2. CONTEXTUAL NAMES: Use names in the Discussion Points only if they add important context (e.g., "Chris clarified that..."). If not, describe the agreed-upon fact.
#     3. LANGUAGE: Write the final executive report strictly in {language}. Do not translate to English unless the notes are in English.
...
#         # Make the owner names bold or styled if needed, although user screenshot just has them normal
